...
export default {
  ...
  // 钩子函数
  mounted() {
    getPicsList({ limit: 5, tab: "weex", page: this.page }).then(response => {
  ...
  methods: {
    infiniteHandler($state) {
      setTimeout(() => {
		// 查数据 
		if(this.len){
        getPicsList({ limit: 5, tab: "weex", page: ++this.page }).then(
          response => {
           		// 把 ajax 获取到的数据 给 topics
				this.topics = this.topics.concat(response.data.data);
				this.len = response.data.data.length;
          }
        );
			$state.loaded();
			$state.complete();
